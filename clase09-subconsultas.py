...
#EN RESUMEN, DIGAMOS QUE PUEDEN HABER 3 TIPOS DE SUBCONSULTAS
#? LAS QUE DEVUELVEN UN VALOR UNICO. PUEDO PONER WHERE = (SUBCONSULTA)
#? LAS QUE DUEVUELVEN UNA COLUMNA . PUEDO PONER WHERE COLUMN (IN, NOT IN ANY, ALL) (SUBCONSULTA)
#? LAS QEU DEVUELVAN UNA TABLA. PUEDO APARTIR DE ELLAS HACER UN INSERT, UPDATE, ETC

#EJEMPLOS DE SUBCONSULTAS
#1) MOSTRAR LOS CLIENTES QUE HAN COMPRADO (EN CANTIDAD) MAS QUE EL PROMEDIO

-- la sub me tiene devolver el promedio de ventas en cantidad
select avg(total_cant_can_cliente) from(
select sum(v.cantidad) as total_cant_can_cliente, c.idcliente from venta v join cliente c on (v.idcliente = c.idcliente) group by c.idcliente) as t;

-- despues hacer un select condicionado

select idcliente, sum(cantidad) from venta group by idcliente having sum(cantidad) > (select avg(total_cant_can_cliente) from(
select sum(v.cantidad) as total_cant_can_cliente, c.idcliente from venta v join cliente c on (v.idcliente = c.idcliente) group by c.idcliente) as t);
